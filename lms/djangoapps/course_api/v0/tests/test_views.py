...
class TestCourseDataMixin(object):
    ...
    def create_test_data(self):
        ...
        self.ASSIGNMENT = ItemFactory.create(
            category="sequential",
        ...
        self.PROBLEM = ItemFactory.create(
            category="problem",
            parent_location=self.ASSIGNMENT.location,
...
class CourseDetailTests(CourseDetailMixin, CourseViewTestsMixin, ModuleStoreTestCase):
    ...
    def test_get_with_depth(self):
        ...
        self.assertValidResponseContent(children[0], self.ASSIGNMENT)
...
class CourseContentListTests(CourseDetailMixin, CourseViewTestsMixin, ModuleStoreTestCase):
    ...
    def test_get(self):
        ...
        self.assertValidResponseContent(response.data[0], self.ASSIGNMENT)
    ...
    def test_get_with_depth(self):
        ...
        self.assertValidResponseContent(child, self.ASSIGNMENT)
...
class CourseContentDetailTests(CourseDetailMixin, CourseViewTestsMixin, ModuleStoreTestCase):
    ...
    def test_get_invalid_course(self):
        ...
        url = reverse(self.view, kwargs={'course_id': self.INVALID_COURSE_ID,
                                         'content_id': unicode(self.ASSIGNMENT.location)})
        response = self.http_get(url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_get(self):
        ...
        The view should return a 200 if the course ID is invalid.
        """
        url = reverse(self.view, kwargs={'course_id': self.COURSE_ID, 'content_id': unicode(self.ASSIGNMENT.location)})
        response = self.http_get(url)
        self.assertEqual(response.status_code, 200)
        self.assertValidResponseContent(response.data, self.ASSIGNMENT)
...
@override_settings(MODULESTORE=TEST_DATA_MOCK_MODULESTORE)
@override_settings(EDX_API_KEY=TEST_API_KEY)
class CourseAssignmentsTests(CourseDetailMixin, CourseViewTestsMixin, ModuleStoreTestCase):
    view = 'course_api:v0:assignments'

    def test_get(self):
        """
        The view should return assignments for a course.
        """
        response = super(CourseAssignmentsTests, self).test_get()

        expected = [
            {
                'id': unicode(self.ASSIGNMENT.location),
                'name': self.ASSIGNMENT.display_name,
